...
export const dashboardStats = catchAsyncErrors(async (req, res, next) => {
  ...
  const totalUserCount = parseInt(totalUsersCountQuery.rows[0].count) || 0;
  ...
  const todayRevenueQuery = await database.query(
    `
        SELECT SUM(total_price) FROM orders WHERE DATE(created_at)=$1 AND paid_at IS NOT NULL
    `,
    [todayDate],
  );
  ...
  const lowStocksProductQuery = await database.query(`
        SELECT name, stock FROM products WHERE stock<=5
    `);

  const lowStocksProducts = lowStocksProductQuery.rows;
  ...
  res.status(200).json({
    ...
    currentMonthSales,
    topSellingProducts,
    lowStockProducts: lowStocksProducts,
    revenueGrowth,
    newUsersThisMonth,
  });
});
