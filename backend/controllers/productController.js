...
export const fetchAllProducts = catchAsyncErrors(async (req, res, next) => {
  ...
  // Query For Fatching new Products
  ...
  // Query For Fatching Top Rated Products(ratings >= 4.5)
  const topRatedQuery = `SELECT p.*, COUNT(r.id) AS review_count
  FROM products p
  LEFT JOIN reviews r ON p.id = r.product_id
  WHERE p.ratings >= 4.5
  GROUP BY p.id
...
export const postProductReview = catchAsyncErrors(async (req, res, next) => {
  ...
  const purchasheCheckQuery = `
    ...
    WHERE o.buyer_id = $1
    AND oi.product_id = $2
    AND (p.payment_status = 'Paid' OR o.order_status = 'Delivered')
    LIMIT 1 
  `;
